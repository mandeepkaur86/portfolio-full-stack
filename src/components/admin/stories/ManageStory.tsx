...
export default function ManageStory(){
    ...
    useEffect(()=>{
        ...
        ApiServices.getAllStory(data).then(
            (res)=>{
                if(res.data.success){
                    ...
                    {data?.map(
                        (el,index)=>(
                            <tr key={index}>
                                <td>{index+1}</td>
                                <td>
                                    <img src={BASE_URL+"/"+(el as any)?.image} style={{height:"200px",width:"200px"}}/>
                                </td>
                                <td>{(el as any)?.name}</td>
                                <td>{(el as any)?.themeId?.name}</td>
                                <td>{(el as any)?.author}</td>
                                <td>{(el as any)?.story}</td>
                                <td>{(el as any)?.description}</td>
                                <td>
                                    <Link to={"/admin/updateStory/"+(el as any)?._id}>
                                    <i className="bi bi-pencil-square text-success fs-3"></i>
                                    </Link>
                                    <i className="bi bi-trash fs-3 text-danger" onClick={()=>{deleteStory((el as any)?._id)}}></i>
                                </td>
                            </tr>
                        )
                    )}
